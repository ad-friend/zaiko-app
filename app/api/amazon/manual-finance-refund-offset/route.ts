 ...
 * reconcile-sales の applyOffsetReconciliation と同様に status のみ reconciled（在庫は触らない）。
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, message: "返金・プラス売上の相殺として消込しました（在庫は変更していません）。" });
  } catch (e: unknown) {

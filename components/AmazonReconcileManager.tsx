  ...
  return (
    <div className="space-y-8">
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-bold text-slate-800 mb-2">自動消込の実行</h2>
        <p className="text-sm text-slate-500 mb-4">
          reconciliation_status = &apos;pending&apos; の注文に対して、新品・セット・中古（1件のみ候補）の自動消込を行います。中古で候補が2件以上の場合は手動確認に回ります。
        </p>
        <button
          type="button"
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-bold text-slate-800 mb-2">未処理注文（手動確認）</h2>
        <p className="text-sm text-slate-500 mb-4">
          status = &apos;manual_required&apos; の注文です。中古在庫候補から1件を選んで「この在庫で確定」してください。
        </p>
        {loading ? (
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50/50 p-4 shadow-sm">
      <div className="mb-3">
        ...
        {order.jan_code && <p className="text-xs text-slate-500">JAN: {order.jan_code}</p>}
      </div>
      {loadingCandidates ? (
        <p className="text-xs text-slate-500">在庫候補を取得中...</p>
      ) : candidates.length === 0 ? (
        <p className="text-xs text-slate-500">該当する中古在庫候補がありません。</p>
      ) : (
        <>
          ...
          <select
            ...
            className="mb-3 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20"
          >
          ...
          <button
            ...
            className={`${buttonClass} w-full bg-primary text-white hover:bg-primary/90 disabled:bg-slate-300 text-sm`}
          >

...
CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_transactions_event_hash ON sales_transactions (amazon_event_hash);
...
COMMENT ON TABLE sales_transactions IS 'Amazon Finances API から取得した売上・手数料・返品・補填データ（重複は amazon_event_hash で防止）。stock_id は本消込で inbound_items.id を紐付け';
